...
Uma variável chamou a atenção desde o tópico "Descrição dos Dados", o *freesulfurdioxide*, por sua descrição dar indícios que este valor está relacionado ao sabor e, consequentemente, a qualidade do vinho. Testando esta relação:  
```{r echo=TRUE, warning=FALSE, message=FALSE}
by(wine_white_dset$freesulfurdioxide, wine_white_dset$quality, median)
```

```{r echo=TRUE, warning=FALSE, message=FALSE}
by(wine_red_dset$freesulfurdioxide, wine_red_dset$quality, median)
```
...
Após a análise gráfica ficou ainda mais clara a diferença entre cada tipo de vinho (tinto ou branco), portanto, vamos selecionar somente um tipo de vinho para uma análise coerente. Além disso, como o número de dados pros vinhos brancos é bem maior do que o para os tintos (aprox. 3 vezes maior), utilizararem os dados que oferecem mais amostras pra treinar e validar nosso modelo.  
...
Identificando os componentes principais do dataset:
