...
__Variáveis dependentes:__ quality  
__Variáveis independentes:__ volatileacidity+residualsugar+density+alcohol  
...
__Variáveis dependentes:__ quality  
__Variáveis independentes:__ PC1+PC2+PC3+PC4+PC6+PC8+PC9+PC10+PC11  

Nota-se no modelo que a melhora no valor de R² é pouco significativa se comparado com os modelos anteriores, por isso, descartaremos este modelo.  
  
### Testando os modelos

```{r echo=TRUE, warning=FALSE, message=FALSE}
predicted4 <- predict(modelo4, testData)
paste0("Modelo4: ",round(mean(predicted4 == testData$quality)*100, digits = 2),"% de acurácia")
```
Como o modelo retorna valores contínuos, há discrepância nos valores preditos dos valores reais em questão das casas decimais. Arrendondaremos então os valores preditos:  
```{r echo=TRUE, warning=FALSE, message=FALSE}
predicted4 <- round(predicted4, digits = 0)
paste0("Modelo4: ",round(mean(predicted4 == testData$quality)*100,digits = 2),"% de acurácia")

predicted6 <- predict(modelo6, testData)
predicted6 <- round(predicted6, digits = 0)
paste0("Modelo6: ",round(mean(predicted6 == testData$quality)*100,digits = 2),"% de acurácia")

# Calcular Componentes Principais pra testData ???
# predictedCP2 <- predict(modeloCP2, testData2)
# predictedCP2 <- round(predictedCP2, digits = 0)
# paste0("ModeloCP2: ",round(mean(predictedCP2 == testData2$quality)*100,digits = 2),"% de acurácia")
```

Os modelos *modelo4* e  *modelo6* apresentam um R² e taxa de erro muito parecidos, demonstrando que a eficácia deles pra explicar a variância dos dados é muito próxima. Os valores preditos no dataset de teste apresentou uma acurácia de aprox. 50% para ambos modelos. Levando tudo isso em conta, __o modelo indicado é o *modelo4* por trabalhar com menos variáveis evitando assim o uso de informações redundantes.__
...
```{r echo=TRUE, warning=FALSE, message=FALSE}
modeloBackTree4 <- rpart(quality ~volatileacidity+residualsugar+density+alcohol, data = trainData, cp = 0.01)
modeloBackTree4
...
### Testando os modelos
```{r echo=TRUE, warning=FALSE, message=FALSE}
predictBackTree <- predict(modeloBackTree, testData)
paste0("modeloBackTree: ",round(mean(predictBackTree == testData$quality)*100, digits = 2),"% de acurácia")
predictBackTree <- round(predictBackTree, digits = 0)
paste0("modeloBackTree: ",round(mean(predictBackTree == testData$quality)*100,digits = 2),"% de acurácia")

predictBackTree4 <- predict(modeloBackTree4, testData)
predictBackTree4 <- round(predictBackTree4, digits = 0)
paste0("modeloBackTree4: ",round(mean(predictBackTree4 == testData$quality)*100,digits = 2),"% de acurácia")
```
